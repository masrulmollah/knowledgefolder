...
with tabs[0]:
    ...
    with st.expander("🏢 Group Identification", expanded=False):
        ...
        g["group_name"] = c1.text_input("Group / parent entity name", g["group_name"])
        g["fy_current"] = c2.text_input("Current fiscal year label", g["fy_current"])
        g["fy_prior"] = c3.text_input("Prior fiscal year label", g["fy_prior"])
        g["reporting_basis"] = st.text_input("Reporting basis", g["reporting_basis"])
    ...
    with st.expander("📄 Revenue (by Segment & Eliminations)", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["rev_segment1"] = c1.number_input(f"Segment 1 — {G('note_seg1_name')} revenue", value=g["rev_segment1"], step=500.0)
        g["rev_segment2"] = c2.number_input(f"Segment 2 — {G('note_seg2_name')} revenue", value=g["rev_segment2"], step=500.0)
        g["rev_segment3"] = c3.number_input(f"Segment 3 — {G('note_seg3_name')} revenue", value=g["rev_segment3"], step=500.0)
        c1,c2,c3 = st.columns(3)
        g["interco_revenue_elim"] = c1.number_input("Intercompany revenue elimination (positive)", value=g["interco_revenue_elim"], step=100.0)
        g["sales_returns"] = c2.number_input("Sales returns & allowances", value=g["sales_returns"], step=100.0)
        g["sales_discounts"] = c3.number_input("Sales discounts", value=g["sales_discounts"], step=100.0)

    with st.expander("📄 Cost of Goods Sold & Interco COGS Elimination", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["cogs_materials"] = c1.number_input("Direct materials", value=g["cogs_materials"], step=500.0)
        g["cogs_direct_labor"] = c2.number_input("Direct labor", value=g["cogs_direct_labor"], step=500.0)
        g["cogs_overhead"] = c3.number_input("Manufacturing overhead", value=g["cogs_overhead"], step=500.0)
        c1,c2,c3 = st.columns(3)
        g["cogs_depreciation"] = c1.number_input("Depreciation in COGS", value=g["cogs_depreciation"], step=100.0)
        g["cogs_inventory_writedown"] = c2.number_input("Inventory write-down", value=g["cogs_inventory_writedown"], step=50.0)
        g["interco_cogs_elim"] = c3.number_input("Intercompany COGS elimination (positive)", value=g["interco_cogs_elim"], step=100.0)
    ...
    with st.expander("📄 Operating Expenses", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["sga_salaries"] = c1.number_input("SG&A — Salaries & benefits", value=g["sga_salaries"], step=500.0)
        g["sga_marketing"] = c2.number_input("SG&A — Marketing", value=g["sga_marketing"], step=100.0)
        g["sga_rd"] = c3.number_input("SG&A — R&D", value=g["sga_rd"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["sga_da"] = c1.number_input("SG&A — Depreciation & amortisation", value=g["sga_da"], step=100.0)
        g["sga_bad_debt"] = c2.number_input("SG&A — Provision for credit losses", value=g["sga_bad_debt"], step=50.0)
        g["sga_other"] = c3.number_input("SG&A — Other", value=g["sga_other"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["sbc"] = c1.number_input("Share-based compensation", value=g["sbc"], step=50.0)
        g["restructuring"] = c2.number_input("Restructuring & severance", value=g["restructuring"], step=50.0)
        g["impairment"] = c3.number_input("Asset impairment", value=g["impairment"], step=50.0)
        g["acquisition_costs"] = st.number_input("Acquisition-related costs", value=g["acquisition_costs"], step=50.0)

    with st.expander("📄 Other Income / Expense, Tax, NCI", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["interest_income"] = c1.number_input("Interest income (group)", value=g["interest_income"], step=50.0)
        g["interest_expense"] = c2.number_input("Interest expense (group)", value=g["interest_expense"], step=100.0)
        g["interco_interest_elim"] = c3.number_input("Intercompany interest elimination (positive)", value=g["interco_interest_elim"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["gain_loss_assets"] = c1.number_input("Gain/(loss) on asset disposals", value=g["gain_loss_assets"], step=50.0)
        g["fx_gain_loss"] = c2.number_input("Foreign exchange gain/(loss)", value=g["fx_gain_loss"], step=50.0)
        g["equity_method_income"] = c3.number_input("Share of profit of associates", value=g["equity_method_income"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["bargain_purchase_gain"] = c1.number_input("Bargain purchase gain on acquisition", value=g["bargain_purchase_gain"], step=50.0)
        g["other_nonop"] = c2.number_input("Other non-operating income/(expense)", value=g["other_nonop"], step=50.0)
        g["eff_tax_rate_pct"] = c3.number_input("Effective tax rate (%)", value=g["eff_tax_rate_pct"], step=0.5, min_value=0.0, max_value=100.0)
        c1,c2 = st.columns(2)
        g["deferred_tax_exp"] = c1.number_input("Deferred tax expense/(benefit)", value=g["deferred_tax_exp"], step=50.0)
        g["nci_net_income"] = c2.number_input("NCI share of net income", value=g["nci_net_income"], step=50.0)
        c1,c2 = st.columns(2)
        g["disc_ops_pretax"] = c1.number_input("Discontinued ops pre-tax income/(loss)", value=g["disc_ops_pretax"], step=50.0)
        g["disc_ops_tax"] = c2.number_input("Tax on discontinued ops", value=g["disc_ops_tax"], step=50.0)
        c1,c2 = st.columns(2)
        g["preferred_divs"] = c1.number_input("Preferred dividends (IS deduction)", value=g["preferred_divs"], step=50.0)
        g["shares_basic"] = c2.number_input("Weighted avg. shares — basic (thousands)", value=g["shares_basic"], step=500.0)
        g["dilutive_shares"] = st.number_input("Dilutive effect — options/RSUs (thousands)", value=g["dilutive_shares"], step=50.0)

    with st.expander("💹 Other Comprehensive Income (OCI)", expanded=False):
        c1,c2 = st.columns(2)
        g["oci_fx_translation"] = c1.number_input("FX translation adjustment", value=g["oci_fx_translation"], step=50.0)
        g["oci_pension_adj"] = c2.number_input("Pension re-measurement (net of tax)", value=g["oci_pension_adj"], step=50.0)
        c1,c2 = st.columns(2)
        g["oci_hedging"] = c1.number_input("Cash-flow hedge reserve movement", value=g["oci_hedging"], step=50.0)
        g["oci_investments"] = c2.number_input("Fair-value movements on financial investments", value=g["oci_investments"], step=50.0)
    ...
    with st.expander("📈 Retained Earnings", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["re_beginning"] = c1.number_input("Opening retained earnings", value=g["re_beginning"], step=500.0)
        g["prior_period_adj"] = c2.number_input("Prior period restatement (net of tax)", value=g["prior_period_adj"], step=50.0)
        g["cumeff_acctg_change"] = c3.number_input("Cumulative effect of accounting change", value=g["cumeff_acctg_change"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["common_divs_declared"] = c1.number_input("Common dividends declared", value=g["common_divs_declared"], step=100.0)
        g["pref_divs_declared"] = c2.number_input("Preferred dividends declared", value=g["pref_divs_declared"], step=50.0)
        g["buyback_re_effect"] = c3.number_input("Treasury buy-back — excess charged to RE", value=g["buyback_re_effect"], step=50.0)

    with st.expander("🏛️ Consolidated Balance Sheet — Current Assets (excl. Cash)", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["bs_str_investments"] = c1.number_input("Short-term investments", value=g["bs_str_investments"], step=100.0)
        g["bs_ar_gross"] = c2.number_input("Trade receivables, gross", value=g["bs_ar_gross"], step=100.0)
        g["bs_allowance"] = c3.number_input("Allowance for expected credit losses", value=g["bs_allowance"], step=50.0)
        g["interco_ar_elim"] = st.number_input("Intercompany receivables eliminated (positive)", value=g["interco_ar_elim"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["bs_inventory_rm"] = c1.number_input("Inventories — raw materials", value=g["bs_inventory_rm"], step=100.0)
        g["bs_inventory_wip"] = c2.number_input("Inventories — WIP", value=g["bs_inventory_wip"], step=100.0)
        g["bs_inventory_fg"] = c3.number_input("Inventories — finished goods", value=g["bs_inventory_fg"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["bs_prepaid"] = c1.number_input("Prepayments", value=g["bs_prepaid"], step=50.0)
        g["bs_other_ca"] = c2.number_input("Other current assets", value=g["bs_other_ca"], step=50.0)
        g["bs_assets_held_for_sale"] = c3.number_input("Assets held for sale", value=g["bs_assets_held_for_sale"], step=50.0)
    ...
    with st.expander("🏛️ Consolidated Balance Sheet — Non-Current Assets", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["bs_land"] = c1.number_input("Land", value=g["bs_land"], step=100.0)
        g["bs_buildings"] = c2.number_input("Buildings", value=g["bs_buildings"], step=100.0)
        g["bs_machinery"] = c3.number_input("Plant, machinery & equipment", value=g["bs_machinery"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["bs_accum_dep"] = c1.number_input("Accumulated depreciation", value=g["bs_accum_dep"], step=100.0)
        g["bs_rou_operating"] = c2.number_input("ROU assets — operating leases", value=g["bs_rou_operating"], step=100.0)
        g["bs_rou_finance"] = c3.number_input("ROU assets — finance leases", value=g["bs_rou_finance"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["bs_goodwill"] = c1.number_input("Goodwill on consolidation", value=g["bs_goodwill"], step=100.0)
        g["bs_intangibles_gross"] = c2.number_input("Other intangibles, gross", value=g["bs_intangibles_gross"], step=100.0)
        g["bs_accum_amort"] = c3.number_input("Accumulated amortisation", value=g["bs_accum_amort"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["bs_equity_method_inv"] = c1.number_input("Investments in associates (equity method)", value=g["bs_equity_method_inv"], step=100.0)
        g["bs_deferred_tax_assets"] = c2.number_input("Deferred tax assets", value=g["bs_deferred_tax_assets"], step=50.0)
        g["bs_other_nca"] = c3.number_input("Other non-current assets", value=g["bs_other_nca"], step=50.0)

    with st.expander("🏛️ Consolidated Balance Sheet — Current Liabilities", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["bs_ap"] = c1.number_input("Trade payables, gross", value=g["bs_ap"], step=100.0)
        g["interco_ap_elim"] = c2.number_input("Intercompany payables eliminated (positive)", value=g["interco_ap_elim"], step=50.0)
        g["bs_accrued_sal"] = c3.number_input("Accrued salaries & benefits", value=g["bs_accrued_sal"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["bs_accrued_int"] = c1.number_input("Accrued interest", value=g["bs_accrued_int"], step=50.0)
        g["bs_itp"] = c2.number_input("Income taxes payable", value=g["bs_itp"], step=50.0)
        g["bs_current_ltd"] = c3.number_input("Current portion of borrowings", value=g["bs_current_ltd"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["bs_current_op_lease"] = c1.number_input("Current operating lease liability", value=g["bs_current_op_lease"], step=50.0)
        g["bs_current_fin_lease"] = c2.number_input("Current finance lease liability", value=g["bs_current_fin_lease"], step=50.0)
        g["bs_deferred_rev_curr"] = c3.number_input("Deferred revenue, current", value=g["bs_deferred_rev_curr"], step=50.0)
        c1,c2 = st.columns(2)
        g["bs_divs_payable"] = c1.number_input("Dividends payable", value=g["bs_divs_payable"], step=50.0)
        g["bs_other_cl"] = c2.number_input("Other current liabilities", value=g["bs_other_cl"], step=50.0)

    with st.expander("🏛️ Consolidated Balance Sheet — Non-Current Liabilities", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["bs_ltd"] = c1.number_input("Long-term borrowings", value=g["bs_ltd"], step=100.0)
        g["bs_nc_op_lease"] = c2.number_input("Non-current operating lease liability", value=g["bs_nc_op_lease"], step=100.0)
        g["bs_nc_fin_lease"] = c3.number_input("Non-current finance lease liability", value=g["bs_nc_fin_lease"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["bs_dtl"] = c1.number_input("Deferred tax liabilities", value=g["bs_dtl"], step=50.0)
        g["bs_pension"] = c2.number_input("Pension & post-employment obligations", value=g["bs_pension"], step=100.0)
        g["bs_deferred_rev_nc"] = c3.number_input("Deferred revenue, non-current", value=g["bs_deferred_rev_nc"], step=50.0)
        g["bs_other_ncl"] = st.number_input("Other non-current liabilities", value=g["bs_other_ncl"], step=50.0)

    with st.expander("🏛️ Consolidated Equity (excl. RE, AOCI, NCI — those are computed)", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["bs_preferred_par"] = c1.number_input("Preference shares, at par", value=g["bs_preferred_par"], step=100.0)
        g["bs_common_par"] = c2.number_input("Ordinary share capital, at par", value=g["bs_common_par"], step=100.0)
        g["bs_apic"] = c3.number_input("Share premium / Additional paid-in capital", value=g["bs_apic"], step=100.0)
        g["bs_treasury"] = st.number_input("Treasury shares, at cost (contra-equity)", value=g["bs_treasury"], step=100.0)
    ...
    with st.expander("💵 Cash Flow — Beginning Cash & Operating Adjustments", expanded=False):
        g["cf_beg_cash"] = st.number_input("Cash, beginning of period", value=g["cf_beg_cash"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["cf_dep_total"] = c1.number_input("Depreciation (group total)", value=g["cf_dep_total"], step=100.0)
        g["cf_amort_total"] = c2.number_input("Amortisation (group total)", value=g["cf_amort_total"], step=50.0)
        g["cf_def_tax"] = c3.number_input("Deferred tax", value=g["cf_def_tax"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_sbc"] = c1.number_input("Share-based compensation", value=g["cf_sbc"], step=50.0)
        g["cf_prov_doubtful"] = c2.number_input("Provision for credit losses", value=g["cf_prov_doubtful"], step=50.0)
        g["cf_impairment_nc"] = c3.number_input("Non-cash impairment", value=g["cf_impairment_nc"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_gain_adj"] = c1.number_input("Remove gain on disposal (negative)", value=g["cf_gain_adj"], step=50.0)
        g["cf_eq_income_adj"] = c2.number_input("Remove equity-method income (negative)", value=g["cf_eq_income_adj"], step=50.0)
        g["cf_eq_distributions"] = c3.number_input("Distributions from associates", value=g["cf_eq_distributions"], step=50.0)

    with st.expander("💵 Cash Flow — Working Capital Changes", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["cf_chg_ar"] = c1.number_input("Change in trade receivables", value=g["cf_chg_ar"], step=50.0)
        g["cf_chg_inv"] = c2.number_input("Change in inventories", value=g["cf_chg_inv"], step=50.0)
        g["cf_chg_prepaid"] = c3.number_input("Change in prepayments", value=g["cf_chg_prepaid"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_chg_oca"] = c1.number_input("Change in other current assets", value=g["cf_chg_oca"], step=50.0)
        g["cf_chg_ap"] = c2.number_input("Change in trade payables", value=g["cf_chg_ap"], step=50.0)
        g["cf_chg_accr"] = c3.number_input("Change in accrued liabilities", value=g["cf_chg_accr"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_chg_itp"] = c1.number_input("Change in income taxes payable", value=g["cf_chg_itp"], step=50.0)
        g["cf_chg_def_rev"] = c2.number_input("Change in deferred revenue", value=g["cf_chg_def_rev"], step=50.0)
        g["cf_chg_other_liab"] = c3.number_input("Change in other liabilities", value=g["cf_chg_other_liab"], step=50.0)
        g["cf_pension_contrib"] = st.number_input("Pension contributions paid", value=g["cf_pension_contrib"], step=50.0)

    with st.expander("💵 Cash Flow — Investing Activities (incl. Acquisitions)", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["cf_capex_ppe"] = c1.number_input("Capex — PP&E", value=g["cf_capex_ppe"], step=100.0)
        g["cf_capex_int"] = c2.number_input("Capex — Intangibles", value=g["cf_capex_int"], step=100.0)
        g["cf_proc_sale_assets"] = c3.number_input("Proceeds from asset disposals", value=g["cf_proc_sale_assets"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_acquisitions"] = c1.number_input("Acquisition of subsidiaries, net of cash", value=g["cf_acquisitions"], step=100.0)
        g["cf_purch_investments"] = c2.number_input("Purchases of investments", value=g["cf_purch_investments"], step=100.0)
        g["cf_mat_investments"] = c3.number_input("Maturities / sales of investments", value=g["cf_mat_investments"], step=100.0)
        g["cf_loans_rp"] = st.number_input("Loans to related parties", value=g["cf_loans_rp"], step=50.0)

    with st.expander("💵 Cash Flow — Financing Activities (incl. NCI Dividends)", expanded=False):
        c1,c2,c3 = st.columns(3)
        g["cf_proc_ltd"] = c1.number_input("Proceeds from borrowings", value=g["cf_proc_ltd"], step=100.0)
        g["cf_repay_ltd"] = c2.number_input("Repayment of borrowings", value=g["cf_repay_ltd"], step=100.0)
        g["cf_proc_rev"] = c3.number_input("Drawdown on revolving credit", value=g["cf_proc_rev"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["cf_repay_rev"] = c1.number_input("Repayment of revolving credit", value=g["cf_repay_rev"], step=100.0)
        g["cf_fin_lease_pmts"] = c2.number_input("Finance lease principal payments", value=g["cf_fin_lease_pmts"], step=50.0)
        g["cf_proc_stock"] = c3.number_input("Proceeds from share issuance", value=g["cf_proc_stock"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["cf_repurch_stock"] = c1.number_input("Share repurchases (treasury)", value=g["cf_repurch_stock"], step=100.0)
        g["cf_divs_common"] = c2.number_input("Dividends paid — common", value=g["cf_divs_common"], step=50.0)
        g["cf_divs_preferred"] = c3.number_input("Dividends paid — preferred", value=g["cf_divs_preferred"], step=50.0)
        c1,c2 = st.columns(2)
        g["cf_divs_nci"] = c1.number_input("Dividends paid to NCI shareholders", value=g["cf_divs_nci"], step=50.0)
        g["cf_debt_issuance"] = c2.number_input("Debt issuance costs paid", value=g["cf_debt_issuance"], step=50.0)
        g["cf_fx_cash"] = st.number_input("FX effect on cash & equivalents", value=g["cf_fx_cash"], step=50.0)
    ...
    with st.expander("🗒️ Notes — Segments, Debt, Pensions, SBC, Leases, Tax, Geography", expanded=False):
        ...
        c1,c2,c3 = st.columns(3)
        g["note_goodwill_seg1"] = c1.number_input("Goodwill — Seg 1", value=g["note_goodwill_seg1"], step=100.0)
        g["note_goodwill_seg2"] = c2.number_input("Goodwill — Seg 2", value=g["note_goodwill_seg2"], step=100.0)
        g["note_goodwill_seg3"] = c3.number_input("Goodwill — Seg 3", value=g["note_goodwill_seg3"], step=100.0)

        st.markdown("**Geographic revenue**")
        c1,c2,c3 = st.columns(3)
        g["note_geo_rev_americas"] = c1.number_input("Americas revenue", value=g["note_geo_rev_americas"], step=100.0)
        g["note_geo_rev_emea"] = c2.number_input("EMEA revenue", value=g["note_geo_rev_emea"], step=100.0)
        g["note_geo_rev_apac"] = c3.number_input("Asia Pacific revenue", value=g["note_geo_rev_apac"], step=100.0)

        st.markdown("**Debt maturities**")
        c1,c2,c3 = st.columns(3)
        g["note_debt_rate"] = c1.number_input("Wtd. avg. borrowing rate (%)", value=g["note_debt_rate"], step=0.1)
        g["note_debt_yr1"] = c2.number_input("Maturing — Year 1", value=g["note_debt_yr1"], step=100.0)
        g["note_debt_yr2"] = c3.number_input("Maturing — Year 2", value=g["note_debt_yr2"], step=100.0)
        c1,c2,c3 = st.columns(3)
        g["note_debt_yr3"] = c1.number_input("Maturing — Year 3", value=g["note_debt_yr3"], step=100.0)
        g["note_debt_yr4"] = c2.number_input("Maturing — Year 4", value=g["note_debt_yr4"], step=100.0)
        g["note_debt_yr5"] = c3.number_input("Maturing — Year 5", value=g["note_debt_yr5"], step=100.0)
        g["note_debt_thereafter"] = st.number_input("Maturing — Thereafter", value=g["note_debt_thereafter"], step=100.0)

        st.markdown("**Pension**")
        c1,c2 = st.columns(2)
        g["note_pension_disc"] = c1.number_input("Discount rate (%)", value=g["note_pension_disc"], step=0.1)
        g["note_pension_ret"] = c2.number_input("Expected return on assets (%)", value=g["note_pension_ret"], step=0.1)
        c1,c2 = st.columns(2)
        g["note_pension_pbo"] = c1.number_input("Projected benefit obligation", value=g["note_pension_pbo"], step=100.0)
        g["note_pension_assets"] = c2.number_input("Fair value of plan assets", value=g["note_pension_assets"], step=100.0)

        st.markdown("**SBC, Leases, Tax, Contingencies**")
        c1,c2,c3 = st.columns(3)
        g["note_sbc_unrec"] = c1.number_input("Unrecognised SBC expense", value=g["note_sbc_unrec"], step=100.0)
        g["note_sbc_period"] = c2.number_input("Wtd. avg. recognition period (yrs)", value=g["note_sbc_period"], step=0.1)
        g["note_lease_op_exp"] = c3.number_input("Operating lease expense", value=g["note_lease_op_exp"], step=50.0)
        c1,c2,c3 = st.columns(3)
        g["note_lease_term"] = c1.number_input("Wtd. avg. remaining lease term (yrs)", value=g["note_lease_term"], step=0.1)
        g["note_lease_rate"] = c2.number_input("Wtd. avg. lease discount rate (%)", value=g["note_lease_rate"], step=0.1)
        g["note_stat_tax_rate"] = c3.number_input("Statutory tax rate (%)", value=g["note_stat_tax_rate"], step=0.5)
        c1,c2 = st.columns(2)
        g["note_contingency_accrued"] = c1.number_input("Legal contingencies accrued", value=g["note_contingency_accrued"], step=50.0)
        g["note_contingency_possible"] = c2.number_input("Reasonably possible losses (disclosed)", value=g["note_contingency_possible"], step=50.0)
        c1,c2 = st.columns(2)
        g["note_rpt"] = c1.number_input("Related-party transactions", value=g["note_rpt"], step=50.0)
        g["note_subsequent"] = c2.number_input("Subsequent event amount (if any)", value=g["note_subsequent"], step=50.0)
        g["note_unrealised_interco_profit"] = st.number_input("Unrealised intercompany profit remaining in inventory", value=g["note_unrealised_interco_profit"], step=50.0)
